...
/*
 * This function called when page load 
 */
function ready_page(){
	$("#divUpdate").hide();
	$("#divAdd").hide();
	...
	$('#btn_update').click(
			function() {
				...
				var uconfirmpassword = $('#uconfirmpassword').val().trim();

				var array_roles_left = new Array();
				$('#uroles_left option').map(function(index, elem) {
					array_roles_left.push({
						role : $(elem).val(),
						role_name : $(elem).text()
				...
				var array_roles_right = new Array();
				$('#uroles_right option').map(function(index, elem) {
					array_roles_right.push({
						role : $(elem).val(),
						role_name : $(elem).text()
				...
				var uuser = {
					...
					password : upassword,
					enable : $('#uenable').val(),
					roles : array_roles_left,
					rightRoles : array_roles_right

				};

				var data = JSON.stringify(uuser);
				console.log(data);
				$.ajax({
					type : "PUT",
					url : "api/user",
					...
					success : function(data) {
						...
						if (data.status == "SUCCESS") {
							...
							table.row("#" + data.result.DT_RowId).data(userForTable).draw();
	...
	$("#uaddrole").click(function() {
		var id;
		var text;
		$('#uroles_right :selected').each(function() {
			id = $(this).val();
			text = $(this).text();

			$("#uroles_left").append($('<option>', {
				value : id,
				text : text
			}));
			$(this).remove();
	...
	$("#uremoverole").click(function() {
		var id;
		var text;
		$('#uroles_left :selected').each(function() {
			id = $(this).val();
			text = $(this).text();

			$("#uroles_right").append($('<option>', {
				value : id,
				text : text
			}));
			$(this).remove();
	...
	$("#aaddrole").click(function() {
		var id;
		var text;
		$('#aroles_right :selected').each(function() {
			id = $(this).val();
			text = $(this).text();

			$("#aroles_left").append($('<option>', {
				value : id,
				text : text
			}));
			$(this).remove();
	...
	$("#aremoverole").click(function() {
		var id;
		var text;
		$('#aroles_left :selected').each(function() {
			id = $(this).val();
			text = $(this).text();

			$("#aroles_right").append($('<option>', {
				value : id,
				text : text
			}));
			$(this).remove();
	...
	$('#button_add').on('click', function() {
		...
		$('#button_del').show();
		$("#aroles_left").empty();
		$.getJSON("api/userrole/list", function(data) {
			$("#aroles_right").empty();
	...
	function edit_refresh(user1) {
		var table = $('#user_table').DataTable();
		$('#json_result_status').text("");
		$('#json_result_error').text("");
		id = table.row('.selected').id();

		$.getJSON("api/user/getDTO/" + id, function(user) {
			console.log(user);
			$("#uusername").val(user.username);
			$("#uiduser").val(user.username);
			$("#u_span_password").text(user.password);
			$("#uenable").val(user.enable.toString());
			$('#uroles_left').empty();
			$.each(user.roles, function(order, object) {
				key = object.role;
			...
			$('#uroles_right').empty();
			$.each(user.rightRoles, function(order, object) {
				key = object.role;
				value = object.role_name;
				$('#uroles_right').append($('<option>', {
					value : key
	...
	$("#btn_create").click(
			function() {
				...
				var aconfirmpassword = $('#aconfirmpassword').val().trim();

				var array_roles_left = new Array();
				$('#aroles_left option').map(function(index, elem) {
					array_roles_left.push({
						role : $(elem).val(),
						role_name : $(elem).text()
				...
				var array_roles_right = new Array();
				$('#aroles_right option').map(function(index, elem) {
					array_roles_right.push({
						role : $(elem).val(),
						role_name : $(elem).text()
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data.status == "SUCCESS") {
							...
							console.log(userForTable);
							table.row.add(userForTable).draw();
